...
import { parseCsvFile, validateTickets } from "../services/csvParser";
...
export function ImportDataPage() {
  ...
  const handleFileUpload = (file: File) => {
    setFileData({ file, uploaded: true, rows: undefined });
    setValidationStatus("idle");
    parseCsvFile(file).then((result) => {
      const { valid } = validateTickets(result.tickets);
      setFileData((prev) => ({ ...prev, rows: result.totalRows || result.tickets.length }));
      setValidationStatus(valid ? "valid" : result.errors.length ? "error" : "valid");
    }).catch(() => {
      setFileData((prev) => ({ ...prev, rows: 0 }));
  ...
  const showUploadZone = stage === "idle" && !fileData.uploaded;
  const showSuccessAndButton = stage === "idle" && fileData.uploaded;
